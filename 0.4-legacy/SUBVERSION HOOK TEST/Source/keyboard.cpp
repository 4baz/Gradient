...
{
	if (a) *a = KeyDown(VK_RETURN) || KeyDown(VK_NUMPAD5);
	if (b) *b = KeyJustUp(VK_BACK) || KeyJustUp(VK_NUMPAD0);
	if (up) *up = KeyDown(VK_UP) || KeyDown(VK_NUMPAD8);
	if (down) *down = KeyDown(VK_DOWN) || KeyDown(VK_NUMPAD2);
	if (r) *r = KeyDown(VK_RIGHT) || KeyDown(VK_NUMPAD6);
	if (l) *l = KeyDown(VK_LEFT) || KeyDown(VK_NUMPAD4);
}
