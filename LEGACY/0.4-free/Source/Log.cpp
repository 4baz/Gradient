...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: Gradient Free");
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "GradientFree.log");
			...
			OutputDebugStringA("INIT: Gradient Free");
			OutputDebugStringA("GradientFree");
		}
		else {
			useFileOutput = false;
		}
