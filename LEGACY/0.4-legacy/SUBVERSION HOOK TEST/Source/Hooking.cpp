...
void CHooking::cleanup()
{
	...
	for (int i = 0; i < m_hooks.size(); i++)
		if (MH_DisableHook(m_hooks[i]) != MH_OK && MH_RemoveHook(m_hooks[i]) != MH_OK)
			Log::Msg("Failed to unhook %p", (void*)m_hooks[i]);
	MH_Uninitialize();
...
{
	Log::Msg("Failed to find %s pattern.", name);
	killProcess();
}
...
void CHooking::FailPatterns(const char* name, Memory::pattern ptn)
{
...
//		"xxxxxxxxxxxxx");
//	ptr = pattern_obj_table.virtual_find(1).get(0).get(4);
//	ptr == nullptr ? killProcess() : CHooking::m_objectHashTable = (void*)ptr;
//	CHooking::m_objectHashTableSectionEnd = pattern_obj_table.get(0).section_end();
